...
if(ndims==0){
  ndims = read.csv(fs::path(data_path, data_ref, "results", "scVI", paste(fname_attach,"scVI", "hyperparams", sep = "_"), ext = "csv"))
...
WNN_bin = combine_Seuratv4(data1 = HVG_dat, data2 = SG_bin_dat,
                               data1_name = "HVG", data2_name = "SG", data1_reduce = TRUE, data2_reduce = TRUE,
                               data1_npcs = 100, data2_npcs=100,
                               data1_dims = ndims, data2_dims = ndims, 
                               n_neighbors = num_nn, smooth = as.logical(WNN_smooth), res = 0.5, res_step=0.1, max_iter=100,
