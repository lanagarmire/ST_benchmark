...
data1 = read.csv(fs::path("data", paste("data_cells", params_df$cells[task_id], proc, 1, params_df$reps[task_id], sep="_"), ext="csv"), row.names=1)
colnames(data1) = paste("cells", 1:ncol(data1), sep="_")
data1_name = "HVG"

data2 = read.csv(fs::path("data", paste("data_cells", params_df$cells[task_id], proc, 2, params_df$reps[task_id], sep="_"), ext="csv"), row.names=1)
colnames(data2) = paste("cells", 1:ncol(data2), sep="_")
data2_name = "SG"
...
save(elapse, file = fs::path("results", "WNN", paste(params_df$cells[task_id], "WNN", params_df$reps[task_id], "time", sep="_"), ext="RData"))
