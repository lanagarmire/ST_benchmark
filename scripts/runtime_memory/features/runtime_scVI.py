...
def main(job_id, data_path, ndims, scVI_error):
    ...
    'load parameters'
    params_name = f"{save_path}/data/features.csv"
    ...
    data_name = params_df.iloc[job_id]['features']
    rep = params_df.iloc[job_id]['reps']
    num_clusters = 8
    hvg_name = f"{save_path}/data/data_features_{data_name}_{method_proc.at[method_name,'proc']}_1_{rep}.csv"
    svg_name = f"{save_path}/data/data_features_{data_name}_{method_proc.at[method_name,'proc']}_2_{rep}.csv"
