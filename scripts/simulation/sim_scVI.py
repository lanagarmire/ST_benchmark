...
def main(job_id, data_path, thres, data_ref, diff_in_cov, svg_pval_thres, num_nn, ndims, scVI_error, SNF_metric, WNN_smooth, CIMLR_k, save_name, n_random_reps):
    ...
    for sd in seed_list:
      ...
      pd.DataFrame.from_dict(tuning_dict).to_csv(f"{save_path}/results/scVI/sim_spat_{spat_lim}_rep_{repid}_{method_name}_hyperparams_{sd}.csv",index = False)

      latent_df = pd.DataFrame(data = adata.obsm["X_scVI"])
      if ndims == 0:
        ndims_optim = tuning_dict["n_latent"]
        latent_df.to_csv(f"{save_path}/results/scVI/sim_spat_{spat_lim}_rep_{repid}_{method_name}_dim_{ndims_optim}_{sd}.csv",index = False,header = False)
      else:
        latent_df.to_csv(f"{save_path}/results/scVI/sim_spat_{spat_lim}_rep_{repid}_{method_name}_dim_{ndims[0]}_{sd}.csv",index = False,header = False)
...
if __name__ == "__main__":
    ...
    main(args.job_id-1, args.data_path, args.thres, args.data_ref, args.diff_in_cov, args.svg_pval_thres, args.num_nn, args.ndims, args.scVI_error, args.SNF_metric, args.WNN_smooth, args.CIMLR_k, args.save_name, args.n_random_reps)
